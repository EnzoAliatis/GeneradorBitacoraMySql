...
const triggerInsert = nombreTabla => {
  return new Promise(async (resolve, reject) => {
    ...
    const verificar = await anadirLinea(`CREATE TRIGGER tr_insert${nombreTabla.toUpperCase()} BEFORE INSERT ON ${nombreTabla} 
    FOR EACH ROW INSERT INTO bitacora(usuario, descripcion, fecha) VALUES (user(), "${nombreTabla} creado", now());
    //`)
    if (verificar) {
      resolve(true)
    } else {
